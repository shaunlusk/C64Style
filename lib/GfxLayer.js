...
C64Style.GfxLayer.prototype._checkForCollisions = function() {
  ...
  for (i = 0; i < this._elements.length; i++) {
    ...
    for (j = 1; j < this._elements.length; j++) {
      ...
      if ((element1.isDirty() && element2.isDirty()) ) continue;
      if ((element1.isHidden() && element2.isHidden()) ) continue;

      if (this._collisionCheck(element1,element2)) {
...
C64Style.GfxLayer.prototype._collisionCheck = function(element1, element2) {
  var collided = a.collidesWith(b);
  var collidedLast = a.collidesWith_LastPosition(b);
  return collided || collidedLast;
};
...
C64Style.GfxLayer.prototype.render = function() {
  ...
  for (i = 0; i < this._dirtyElements.size(); i++) {
    this._dirtyElements.getByIndex(i).getElement().clear();
  }
