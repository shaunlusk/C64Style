...
C64Style.GfxLayer = function(screenContext, canvas, config) {
  config = config || {};
...
C64Style.GfxLayer.prototype._checkBorderCollision = function(element,time) {
  ...
  if (element.getX() + element.getCollisionBoxWidth() > this.getCanvas().width) {
  ...
  if (element.getY() + element.getCollisionBoxHeight() > this.getCanvas().height) {
