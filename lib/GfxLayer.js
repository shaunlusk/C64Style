...
C64Style.GfxLayer.prototype._checkBorderCollision = function(element,time) {
  if (element.collidesWithX(0)) {
    this.getEngineContext().notify(new C64Style.Event("ELEMENT_HIT_LEFT_EDGE", element, time));
  }
  if (element.collidesWithX(this.getCanvas().width)) {
    this.getEngineContext().notify(new C64Style.Event("ELEMENT_HIT_RIGHT_EDGE", element, time));
  }
  if (element.collidesWithY(0)) {
    this.getEngineContext().notify(new C64Style.Event("ELEMENT_HIT_TOP_EDGE", element, time));
  }
  if (element.collidesWithY(this.getCanvas().height)) {
    this.getEngineContext().notify(new C64Style.Event("ELEMENT_HIT_BOTTOM_EDGE", element, time));
  }
};
