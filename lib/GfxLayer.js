...
* Checks for elements colliding with the layer boundary and emits events accordingly:
* <ul>
...
C64Style.GfxLayer.prototype._checkBorderCollision = function(element,time) {
  if (element.getCollisionBoxX() <= 0) {
    this.getScreenContext().notify(new C64Style.Event(C64Style.EventType.ELEMENT_HIT_LEFT_EDGE, {layer:this, element:element}, time));
  }
  if (element.getCollisionBoxX() + element.getCollisionBoxWidth() > this.getCanvas().width) {
    this.getScreenContext().notify(new C64Style.Event(C64Style.EventType.ELEMENT_HIT_RIGHT_EDGE, {layer:this, element:element}, time));
  }
  if (element.getCollisionBoxY() <= 0) {
    this.getScreenContext().notify(new C64Style.Event(C64Style.EventType.ELEMENT_HIT_TOP_EDGE, {layer:this, element:element}, time));
  }
  if (element.getCollisionBoxY() + element.getCollisionBoxHeight() > this.getCanvas().height) {
    this.getScreenContext().notify(new C64Style.Event(C64Style.EventType.ELEMENT_HIT_BOTTOM_EDGE, {layer:this, element:element}, time));
  }
};
