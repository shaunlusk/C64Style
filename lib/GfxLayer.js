...
C64Style.GfxLayer.prototype._checkBorderCollision = function(element,time) {
  if (element.getX() * this.getScreenContext().getScaleX() <= 0) {
    this.getScreenContext().notify(new C64Style.Event("ELEMENT_HIT_LEFT_EDGE", element, time));
  }
  if (element.getX() * this.getScreenContext().getScaleX() + element.getCollisionBoxWidth() > this.getCanvas().width) {
    this.getScreenContext().notify(new C64Style.Event("ELEMENT_HIT_RIGHT_EDGE", element, time));
  }
  if (element.getY() * this.getScreenContext().getScaleY() <= 0) {
    this.getScreenContext().notify(new C64Style.Event("ELEMENT_HIT_TOP_EDGE", element, time));
  }
  if (element.getY() * this.getScreenContext().getScaleY() + element.getCollisionBoxHeight() > this.getCanvas().height) {
    this.getScreenContext().notify(new C64Style.Event("ELEMENT_HIT_BOTTOM_EDGE", element, time));
  }
};
...
C64Style.GfxLayer.prototype._getElementsForMouseEvent = function(x, y) {
  var elements = [];
  for (var i = 0; i < this._elements.length; i++) {
    var element = this._elements[i];
    if (element.collidesWithCoordinates(x, y)) {
