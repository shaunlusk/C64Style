...
C64Style.Screen.prototype._setupEventListeners = function() {
  ...
  window.document.addEventListener("visibilitychange", this.handleVisibilityChange.bind(this), false);
};
...
C64Style.Screen.prototype.getBackgroundColor = function(color) {
  return this._targetDiv.style.backgroundColor;
};
...
C64Style.Screen.prototype.render = function(time) {
  ...
  if (this._mouseMoved) {
    var elements = this._getElementsForMouseEvent(this._mouseX, this._mouseY);
    this.notify(
...
C64Style.Screen.prototype.handleMouseEvent = function(e) {
  ...
  var elements = this._getElementsForMouseEvent(x, y);
...
C64Style.Screen.prototype._getElementsForMouseEvent = function(x, y) {
  var elements = [];
  for (var i = 0; i < this._layers.length; i++) {
    elements.push(this._layers[i]._getElementsForMouseEvent(x,y));
  }
  return elements;
};
