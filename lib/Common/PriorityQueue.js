...
PriorityQueue.prototype.sort = function() {
  ...
  for (var i = this._heapSize - 1; i >= 1; i--) {
    this._swap(0,i);
    this._maxHeapify(0, (this._heapSize - (this._heapSize-i)) /*this._heapSize*/ );
  }
};
...
PriorityQueue.prototype._maxHeapify = function( i, size, dir ) {
  ...
  if ( left < /*this._heapSize*/ size && this._a[left].compareTo(this._a[i]) === dir ) {
    largest = left;
  } else {
    largest = i;
  }

  if ( right < /*this._heapSize*/ size && this._a[right].compareTo(this._a[largest]) === dir ) {
    largest = right;
  }

  if ( largest != i ) {
    this._swap(i,largest);
    this._maxHeapify( largest, /*this._heapSize*/ size, dir );
  }
};
