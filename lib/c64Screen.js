...
C64Style.Screen.prototype.on = function(event, callback) {
  if (!this._eventListeners[event]) {
    this._eventListeners[event] = [];
  }
  var id = this._eventListeners[event].length;
...
C64Style.Screen.prototype.render = function(time) {
  ...
  if (debug && this.frameCount < 10) {
    this.frameCount++;
    console.log(Math.floor(performance.now()) + " *** Start Frame ***");
  }
