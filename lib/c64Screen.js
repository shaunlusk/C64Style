...
C64Style.Screen.prototype._prepareDiv = function() {
  ...
  this._targetDiv.style.border = this._config.border || "20px solid " + C64Style.Color.LIGHTBLUE;
};
...
C64Style.Screen.prototype.setBorderColor = function(color) {
  this._targetDiv.style.border = "20px solid " + color;
};
...
C64Style.Screen.prototype.setPaused = function(boolean) {
  ...
  this.notify(
    new C64Style.Event(this._paused ? "ENGINE_PAUSED" : "ENGINE_RESUMED")
  );
...
C64Style.Screen.prototype.render = function(time) {
  ...
  this.notify(
    new C64Style.Event("BEFORE_RENDER", null, time)
  );
  ...
  this.notify(
    new C64Style.Event("AFTER_RENDER", null, time)
  );
...
C64Style.Screen.prototype._render = function(time,diff) {
  for (var i = 0; i < this._uiLayers.length; i++) {
    this._uiLayers[i].render(time,diff);
  }
};
